      ...
      <div>
        ${config.choices.map((props, index) => html`
          <div class="Space Space--start">
            ${render(Nominee, type, props, index)}
          </div>
        `)}
          ...
          <div class="Text">
              ...
              ${config.choices.map((props, index) => html`
                <li class="Form-field">
                  <label class="Form-pair">
                    <input type="radio" name="${config.id}" value="${props.value}" required="required" class="Form-toggle" onchange=${onchange} checked="${data[config.id] === props.value}">
            ...
            <div class="Grid" style="margin-top: 40px;">
              ...
              <div class="Grid-cell">
                ...
                <button type="submit" class="${className('Button', {[`Button--${config.button.color}`]: config.button.color})} js-submit" style="margin-left: 16px;">${config.button.text}</button>
              </div>
            </div>
          </div>
        </form>
