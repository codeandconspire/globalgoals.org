...
module.exports = function createView(render, title) {
  return function view(state, emit) {
    ...
    if (!state.error) {
      ...
      content = errors[status](state, state.error);
    }
    ...
    return html`
      <body class="View js-view">
        <div class="View-header" id="view-header">
          ${ !state.error || state.error.status < 500 ? header(state, emit, goal) : null }
        </div>
        ${ content }
        <div class="View-footer u-transformTarget">
          ${ !state.error || state.error.status < 500 ? footer(state) : null }
        </div>
