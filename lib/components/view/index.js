  ...
  return function wrapper(state, emit) {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (state.params.goal) {
        ...
        rootElm.classList.add('u-bg' + state.params.goal);
      } else {
        resetFavicon();
    ...
    return html`
      <body class="View js-view ${state.params.goal ? 'u-bg' + state.params.goal : '' } ${ state.navigationOpen ? 'is-hiddenForNavigation' : ''}">
        <div class="View-container">
          <div class="View-header">
            ${ header(state, state.params.goal, emit) }
          </div>
          ${ content }
          <div class="View-footer">
            ${ footer(state, state.params.goal) }
          </div>
        </div>

        ${ process.env.NODE_ENV !== 'development' ? html`
          <script src="https://cdn.polyfill.io/v2/polyfill.min.js?features=default,Array.prototype.includes,Array.prototype.findIndex"></script>
        ` : null }
      </body>
    `;
  };
