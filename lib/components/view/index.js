...
module.exports = function createView (name, render, title) {
  return function view (state, emit) {
    ...
    if (!state.error) {
      try {
        ...
        content = (errors[err.status] || errors['500'])(err)
    ...
    return html`
      <body class="View js-view">
        ...
        ${state.ui.sharing ? share(state.ui.sharing, emit) : null}
        ...
        ${!inBrowser && !state.isStatic ? html`
          <script>window.initialState = ${raw(JSON.stringify(state).replace(/[\u2028\u200B-\u200D\uFEFF]/g, ''))}</script>
        ` : null}
      </body>
    `
  }
