...
module.exports = class Background extends Component {
  ...
  createElement () {
    return html`
      <div class="Hero-bg Hero4 ${this.tight ? 'Hero4--tight' : ''}" id="hero-bg-4">
        <div class="Hero4-letters">
          ${LETTERS.map((letter, index) => html`
            <div class="Hero4-letter Hero4-letter--${letter} ${(index + 1) % 4 ? '' : 'Hero4-letter--dark'} js-letter" onclick=${letter === 'm' ? easter : null}>
              ${ALIAS[letter] || letter}
            </div>
          `)}
        </div>
