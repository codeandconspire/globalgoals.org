...
module.exports = function createLink(state, goal, emit) {
  return component({
    ...
    render(doc) {
      if (!doc) {
        return html`
          <a id="goal-grid-item-${ goal }" class="GoalGrid-item GoalGrid-item--${ goal } ${ state.goals.isLoading ? 'is-loading' : '' }"  href="${ resolve(state.routes.goal, { goal: goal, referrer: state.params.referrer }) }">
      ...
      return html`
        <a id="goal-grid-item-${ goal }" class="GoalGrid-item GoalGrid-item--${ goal }" href="${ resolve(state.routes.goal, { goal: doc.data.number, slug: doc.uid, referrer: state.params.referrer }) }">
          <div class="GoalGrid-bg u-bg${ goal }">
            <div class="GoalGrid-content">
              <div class="GoalGrid-icon js-icon">
