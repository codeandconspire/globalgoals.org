...
module.exports = class Link extends Component {
  constructor (id, state, emit, goal) {
    super(id)
    this.state = state
    this.emit = emit
    this.goal = +goal
  ...
  load (element) {
    ...
    var swiper = new window.Swiper('.global--goals__swiper-slider', {
      pagination: {
    ...
    swiper.on('slideChange', function (swiper) {
      var els = document.querySelectorAll('.swiper-pagination')
      if (swiper.realIndex === 1) {
        for (var i = 0; i < els.length; i++) {
          const el = els[i]
          el.classList.add('pagination__dark')
        }
      } else {
        for (var i = 0; i < els.length; i++) {
          const el = els[i]
          el.classList.remove('pagination__dark')
        }
      }
    })
  }
  ...
  createElement (goal, doc) {
    ...
    return html`
      <a class="GoalGrid-item GoalGrid-item--${goal} ${goal === 7 ? ' GoalGrid-item--light' : ''}" href="${href}" id="${this._name}">
        ${doc ? html`
          <div class="GoalGrid-bg u-bg${goal}" id="${this._name}-bg">
            ...
            <div class="GoalGrid-animation" id="${this._name}-animation">
