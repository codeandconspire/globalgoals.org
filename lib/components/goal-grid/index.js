...
module.exports = class GoalGrid extends Component {
  ...
  createElement (doc) {
    ...
    function largeBanner (props) {
      ...
      const image = props.image && props.image.url

      const content = html`
        <div class="GoalGrid-bg">
          <div class="GoalGrid-content">
            ${image ? card(state, emit, asBannerCard(props), { fill: props.color || true, growingText: true }) : html`
              <div class="GoalGrid-details">
                <div class="GoalGrid-desc Text Text--growingLate">
                ...
                ${props.link_text ? html`<span class="GoalGrid-button">${props.link_text}</span>` : null}
              </div>
            `}
          </div>
        </div>
...
function asBannerCard (props) {
  ...
  if (props.link.link_type === 'Document') {
    url = href(props.link)
  } else if (props.link.link_type === 'Web') {
    url = props.link.url
  } else if (props.link.link_type === 'Media') {
    url = props.link.url
  } else {
    url = href(props.link)
  }
