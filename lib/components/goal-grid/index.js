...
function largeBanner (props) {
  ...
  if (props && props.link_text) {
    ...
    return html`
      <a class="GoalGrid-item GoalGrid-item--cta" href="${isDocument ? href(props.link) : props.link.url}" target="${isExternal ? '_blank' : '_self'}" rel="${isExternal ? 'noopener noreferrer' : ''}">
        ${content}
      </a>
    `
  }
