...
module.exports = component({
  ...
  update (element, [state]) {
    ...
    return state.goals.items.length > this.length && !inTransition
  },
  render (state, emit, doc) {
    ...
    return html`
      <div class="GoalGrid ${state.layout ? 'GoalGrid--layout GoalGrid--layout' + state.layout : ''}">
        ${goals.map((doc, index) => {
          if (!links[index]) {
...
function largeBanner (props) {
  ...
  if (props && props.link_text) {
    return html`
      <a class="GoalGrid-item GoalGrid-item--cta" href="${resolve(props.link)}">
        ${content}
      </a>
    `
  }
...
function smallBanner (props) {
  ...
  if (props && props.link_text) {
    return html`
      <a class="${bannerClassName}" href="${resolve(props.link)}">${content}</a>
    `
  }
  return html`<div class="GoalGrid-item GoalGrid-item--cta">${content}</div>`
}
...
function resolve (link) {
  if (link.link_type === 'Any') return '#contribute'
  if (link.link_type === 'Document') return href(link)
  return link.url
}
