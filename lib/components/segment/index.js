    ...
    <div class="Segment" id="segment-${ props.id }">
      <figure class="Segment-figure">
        ...
        ${ props.image.url ? image(props.image, color) : html`
          <span class="Segment-fallback u-bg${ color[0].toUpperCase() + color.substr(1) }"><span class="Segment-fallbackText">${ props.fallback || null }</span></span>
        ` }
      </figure>
...
function image(props, color) {
  ...
  const srcset = props['2x'] ? `srcset="${ props['2x'].url } 2x"` : null;

  return html`
    <img class="Segment-image u-bg${ color[0].toUpperCase() + color.substr(1) }" src="${ url }" width="${ width }" height="${ height }" alt="${ alt || '' }" srcset="${ srcset }" />
  `;
}
