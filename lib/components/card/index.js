...
function card (state, emit, props, opts = {}) {
  ...
  let sizes = props.sizes || SIZES
  if (opts.horizontal) sizes = sizes.slice().concat('large')

  const img = props.image.url ? image(props.image, sizes) : null
  ...
  if (opts.fill) {
    ...
    if (bgColor) {
      style = opts.banner ? '' : `background-color:${bgColor};`
  ...
  if (opts.banner) {
    cardClassList += ' Card--banner'
    cardClassList += ' Card--fill'
    cardClassList += ' Card--linked'
    cardClassList += ' Card--dark'
  }
...
function getTextClassList (opts) {
  return className('Text', {
    ...
    'Text--growingLate': opts.growingText,
    'Text--adaptive': opts.fill || opts.banner
  })
}
