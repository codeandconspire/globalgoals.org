...
const props = {
  ...
  update(element, [ state ]) {
    const toggle = state.navigationOpen ? 'add' : 'remove';
    document.documentElement.classList[toggle]('has-menu');

    const routeChanged = state.routeName !== this.route;
    const expandedChanged = state.navigationOpen !== this.isOpen;
    ...
    this.isOpen = state.navigationOpen;
  ...
  render(state, emit, goal) {
    ...
    const linkOnClick = event => {
      emit(state.events.PUSHSTATE, event.currentTarget.href);
      event.preventDefault();
    };

    const goBack = event => {
      emit('transitions:popstate', event.currentTarget.href);
      event.preventDefault();
    };
    ...
    const classNames = {
      ...
      'is-open': state.navigationOpen,
      'is-fromGoalGrid': this.hasBack
    };

    return html`
      <header class="${ className('Header', classNames) }" style="${ opacity ? `opacity: ${ opacity }` : '' }" id="navigation">
        <div class="Header-bar">
          ...
          <div class="View-section">
            <div class="Header-content js-content" style="${ transforms.content ? `transform: ${ transforms.content }` : '' }">
              ...
              <a class="Header-button Header-button--toggle" href="#navigation" draggable="false" onclick=${ toggle } role="button" data-toggle="collapse" data-target="#main-navigation" aria-controls="main-navigation" aria-expanded="${ state.navigationOpen ? 'true' : 'false' }">
                ...
                <ul class="Header-list js-list" style="${ transforms.list ? `transform: ${ transforms.list }` : '' }">
                  ${ links.map(item => item(state)).map(item => html`
                    <li class="Header-item">
                      <a class="Header-button Header-button--link ${ item.isCurrent ? 'is-current' : '' } js-headerLink" onclick=${ item.external ? '""' : linkOnClick } href="${ item.href }">
                        ${ __(item.title) }
