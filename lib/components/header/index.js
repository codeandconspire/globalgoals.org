...
const props = {
  ...
  render(state, emit, adaptive) {
    ...
    const goBack = (e) => {
      e.preventDefault();
      emit(state.events.POPSTATE, e.currentTarget.href);
    };
    ...
    return html`
      <header class="Header ${ adaptive ? 'Header--adaptive' : '' } ${ state.navigationOpen ? 'is-open' : '' } ${ fadeIn ? 'is-fadingIn' : '' }" style="${ opacity ? `opacity: ${ opacity }` : '' }" id="navigation" role="banner">
