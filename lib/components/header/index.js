...
const props = {
  ...
  load(element, state, emit) {
    ...
    element.addEventListener('wheel', onTouchMove);

    this.unload = function() {
      window.removeEventListener('resize', onResize);
      window.removeEventListener('scroll', onScroll);
      element.removeEventListener('touchmove', onTouchMove);
      element.removeEventListener('wheel', onTouchMove);
    };
  },
            ...
            <div class="Header-content js-content" style="${ transforms.content ? `transform: ${ transforms.content }` : '' }">
              ...
              <nav class="Header-nav" id="main-navigation">
                <ul class="Header-list js-list" style="${ transforms.list ? `transform: ${ transforms.list }` : '' }">
                  ${ links.primary.map(item => item(state)).map(item => html`
                ...
                <ul class="Header-list Header-list--secondary">
                  ${ links.secondary.map(item => item(state)).map(item => html`
                    <li class="Header-item">
                      <a class="Header-button Header-button--link" onclick=${ linkOnClick } href="${ item.href }">
                        ${ item.title }
                      </a>
                    </li>
                  `) }
  ...
  handleViewportSize(state, emit) {
    ...
    if (this.onGoalPage) {
      ...
      if (state.routeName === 'home') {
        this.animation.start = (state.ui.headerOffset || vh()) - 100;
      }
    }
  ...
  handleScroll(state) {
    if (state.ui.navigationOpen || !this.shouldTransform) {
      return;
    }
...
Object.defineProperty(header, 'height', {
  get() {
    return props.elements.fill.getBoundingClientRect().height;
  }
});
