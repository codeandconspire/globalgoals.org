...
const props = {
  ...
  render(state, emit, adaptive) {
    ...
    return html`
      <header class="Header ${ adaptive ? 'Header--adaptive' : '' } ${ state.navigationOpen ? 'is-open' : '' } ${ inHistory ? 'is-inHistory' : '' }" style="${ opacity ? `opacity: ${ opacity }` : '' }" id="navigation" role="banner">
