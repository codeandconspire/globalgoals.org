...
const props = {
  ...
  load(element, state) {
    ...
    const onResize = debounce(this.prepairScroll.bind(this), 500);
  ...
  render(state, emit, goal) {
    ...
    if (HAS_WINDOW) {
      ...
      inHistory = state.routeFromGrid;
      state.routeFromGrid = false;
    ...
    return html`
      <header class="Header ${ this.adaptive ? 'Header--adaptive' : '' } ${ adaptiveDarken ? 'Header--adaptiveDarken' : '' } ${ state.navigationOpen ? 'is-open' : '' } ${ inHistory ? 'is-inHistory' : '' }" style="${ opacity ? `opacity: ${ opacity }` : '' }" id="navigation" role="banner">
        <div class="Header-bar">
          ...
          <div class="View-section">
            <div class="Header-content js-content" style="${ transforms.fill ? `transform: ${ transforms.content }` : '' }">
              ${ inHistory ? html`
                <a class="Header-back" onclick=${ goBack } href="/">
                  ${ __('Back to Goals') }
                </a>
              ` : null }
