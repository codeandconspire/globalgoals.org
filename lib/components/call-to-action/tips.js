  ...
  return html`
    <ul class="Grid">
      ${ doc.data.tips.map((tip, index) => html`
        <div class="Grid-cell Grid-cell--md1of2 Grid-cell--appear" id="${ doc.id }-tip-${ index }" style="animation-delay: ${ index * 100 }ms;">
          <div class="CallToAction-tip">
            <svg viewBox="0 0 64 64" width="64" height="64" class="CallToAction-tipIcon">
            ...
            <div class="CallToAction-tipText">
              <div class="Text Text--growing Text--compact">
                ${ tip.title ? html`<strong>${ asText(tip.title) }</strong>` : null }
                ${ asElement(tip.text, doc => href(state, doc)) }
              </div>
            </div>
          </div>
        </div>
