...
module.exports = function createCallToAction (view) {
  ...
  return component({
    ...
    render (state, emit, { doc, heading = {} }) {
      const props = Object.assign(this.props, state.ui.cta)
      const panels = Object.values(allPanels).map(fn => fn(state, doc, emit))
          ...
          ${panels.map(panel => {
            ...
            return html`
              <div class="CallToAction-block" id="${id}-container">
                <a href="${panel.href}" onclick=${toggle(panel.id, true)} class="CallToAction-button CallToAction-button--expandable CallToAction-button--row ${isExpanded ? 'is-expanded' : ''}" role="tab" aria-expanded="${isExpanded ? 'true' : 'false'}" aria-controls="${id}">
                  ${panel.title}
                </a>
