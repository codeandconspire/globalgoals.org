...
module.exports = function createCallToAction(view) {
  ...
  return component({
    ...
    render(state, doc, emit) {
      const props = Object.assign(this.props, state.ui.cta);
      const panels = PANELS.map(panel => panel(state, doc, emit));
      ...
      const toggle = (id, invert) => event => {
        ...
        const done = () => emit('ui:cta:toggle', id);
        ...
        if (isSmall && (!props.expanded || !id)) {
          const panel = document.getElementById(`call-to-action-${ id || props.expanded }`);
