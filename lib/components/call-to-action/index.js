...
const PANELS = [
  (state, emit) => ({
    ...
    content: html`
      <div class="Grid">
        ${ state.goals.items.filter(goal => goal.data.media.length).map(goal => html`
          <div class="Grid-cell Grid-cell--1of2 Grid-cell--md1of3">
            ${ poster(state, goal, goal.data.media[0], emit) }
          </div>
        `) }
      </div>
    `
...
module.exports = component({
  ...
  render(state, emit) {
    const { props } = this;
    const panels = PANELS.map(panel => panel(state, emit));
