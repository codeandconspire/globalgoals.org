...
module.exports = class Organisations extends Component {
  constructor (id, state, emit, doc) {
    ...
    this.state = state
    this.emit = emit
    this.orgs = this.state.pages.items.filter(isOrg)
  ...
  update () {
    return this.state.pages.items.filter(isOrg).length !== this.orgs
  }

  createElement (doc) {
    let orgs = this.orgs = this.state.pages.items.filter(isOrg).sort(byName)
...
function byName (a, b) {
  return asText(a.data.title) < asText(b.data.title) ? -1 : 1
}
