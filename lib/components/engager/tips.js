...
module.exports = class Tips extends Component {
  static title (doc) {
    return doc.type === 'goal' ? __('Things to do') : __('Questions & Answers')
  }

  static href (doc) {
    return resolve(routes[doc.type === 'goal' ? 'goal_tips' : 'faq'], doc)
  }

  static identity (doc) {
    return `engager-${doc.id}-tips`
  }
        ...
        ${doc.data.tips.map((tip, index) => html`
          <div class="Grid-cell Grid-cell--md1of2" id="${doc.id}-tip-${index}">
            <div class="Engager-tip">
              <svg viewBox="0 0 64 64" width="64" height="64" class="Engager-tipIcon u-color${doc.data.number}">
              ...
              <div class="Engager-tipText">
                <div class="Text">
                  ...
                  ${asElement(tip.text, href, serialize)}
                </div>
              </div>
            </div>
          </div>
