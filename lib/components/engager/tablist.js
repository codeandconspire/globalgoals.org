  ...
  return component({
    ...
    render (props, panels, toggle, emit) {
      ...
      const expand = id => event => {
        ...
        const root = document.getElementById('engager-tablist')
      ...
      const moveTo = id => event => {
        ...
        document.getElementById(`engager-${props.expanded}`).classList.add('is-hidden')
      ...
      return html`
        <div class="${className('Engager-tablist js-tablist', states)}" role="tablist" aria-expanded="${props.expanded ? 'true' : 'false'}" id="engager-tablist">
        ...
        return html`
          <div class="Engager-tabs" id="cta-tablist-tabs">
            ${panels.map(panel => {
              ...
              return html`
                <a href="#engager-${panel.id}" onclick=${moveTo(panel.id)} class="${className('Engager-tab', states)}" role="tab" aria-expanded="${isExpanded ? 'true' : 'false'}" aria-controls="engager-${panel.id}">
                  ${panel.title}
                </a>
              `
            })}
      ...
      function buttons (expanded = props.expanded) {
        return panels.map(panel => {
          return html`
            <a href="${panel.href}" onclick=${expand(panel.id)} class="${className('Engager-button', {'js-active': expanded === panel.id})}" role="tab" aria-expanded="${expanded === panel.id ? 'true' : 'false'}" aria-controls="engager-${panel.id}">
