  ...
  return component({
    ...
    render (props, panels, toggle, emit) {
      ...
      const expand = id => event => {
        ...
        const root = document.getElementById('contribute-tablist')
      ...
      const moveTo = id => event => {
        ...
        // TODO: Should trigger a proper render
        document.getElementById(`contribute-${props.expanded}`).classList.add('is-hidden')
      ...
      return html`
        <div class="${className('Engager-tablist js-tablist', states)}" role="tablist" aria-expanded="${props.expanded ? 'true' : 'false'}" id="contribute-tablist">
      ...
      function tabs (expanded = props.expanded, firstPick = props.firstPick) {
        return html`
          <div class="Engager-tabs" id="cta-tablist-tabs">
            ${panels.map(panel => {
              ...
              return html`
                <a href="#contribute-${panel.id}" onclick=${moveTo(panel.id)} class="${className('Engager-tab', states)}" role="tab" aria-expanded="${isExpanded ? 'true' : 'false'}" aria-controls="contribute-${panel.id}">
                  ${panel.title}
                </a>
              `
            })}
      ...
      function buttons (expanded = props.expanded) {
        return panels.map(panel => {
          return html`
            <a href="${panel.href}" onclick=${expand(panel.id)} class="${className('Engager-button', {'js-active': expanded === panel.id})}" role="tab" aria-expanded="${expanded === panel.id ? 'true' : 'false'}" aria-controls="contribute-${panel.id}">
