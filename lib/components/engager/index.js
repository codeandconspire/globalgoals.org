      ...
      <div class="Engager ${goal ? `Engager--${goal}` : ''} ${this.expanded ? 'is-expanded' : ''}" id="${Engager.identity(doc)}">
        ...
        <div class="Engager-tabs js-tabs">
          ...
          ` : PANELS.map(Panel => {
            ...
            return html`
              <a href="${Panel.href(doc)}" onclick=${onexpand(id)} class="${className('Engager-button', {'js-active': this.expanded === id})}" role="tab" aria-expanded="${this.expanded === id ? 'true' : 'false'}" aria-controls="${id}-thing">
        ...
        ${inBrowser ? PANELS.map(Panel => {
          ...
          return html`
            <div class="Engager-block">
              <a href="${Panel.href(doc)}" onclick=${ontoggle(id)} class="Engager-button Engager-button--row ${isExpanded ? 'is-expanded' : ''}" role="button" aria-expanded="${isExpanded ? 'true' : 'false'}" aria-controls="${id}-thing">
                <span class="Engager-buttonText">${Panel.title(doc)}</span>
              </a>

              <div class="Engager-thing ${isExpanded ? 'is-expanded' : ''}" role="tabpanel" aria-expanded="${isExpanded ? 'true' : 'false'}" id="${id}-thing">
