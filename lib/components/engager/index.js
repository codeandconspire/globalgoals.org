          ...
          ${panels.map(panel => {
            ...
            return html`
              <div class="Engager-block" id="${id}-container">
                <a href="${panel.href}" onclick=${toggle(panel.id, true)} class="Engager-button Engager-button--row ${isExpanded ? 'is-expanded' : ''}" role="tab" aria-expanded="${isExpanded ? 'true' : 'false'}" aria-controls="${id}">
