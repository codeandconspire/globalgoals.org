...
const label = (num, darken) => createLabel(num, darken);
...
function createLabel(number, darken) {
  ...
  return html`
    <svg class="Goal-label ${ darken ? 'Goal-label--darken' : '' }" viewBox="0 0 200 ${ viewBoxHeight }">
      <g style="transform: scale(0.94)">
        <text class="Goal-number" font-size="59.4" x="${ digitPos }" y="24.2" fill="currentColor" text-anchor="middle" alignment-baseline="middle">${ number }</text>
        <text class="Goal-text" font-size="24" y="-7" fill="currentColor" text-anchor="start" alignment-baseline="hanging">
          ${ lines.map(line => {
