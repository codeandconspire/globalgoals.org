...
module.exports = class Instagram extends Component {
  constructor (id, state, emit) {
    super(id)
    this.state = state
    this.emit = emit
    this.locals = this.state.instagram
  }

  static identity (photos, cols, { user, hashtag }) {
    return ['instagram', user, hashtag].join('-')
  }
  ...
  render (photos, cols, { user, hashtag }) {
    this.photos = photos

    if (!photos && inBrowser && !this.locals.isLoading) {
      this.emit('instagram:fetch', { user, hashtag })
    }
        ...
        <div class="Card-content u-bgGrayDark">
          <div class="Card-body">
            <div class="Instagram Instagram--size${cols}">
              <svg class="Instagram-icon" viewBox="0 0 504 504" width="504" height="504">
          ...
          <a class="Card-link" href="${link}" target="_blank" rel="noopener noreferrer">
            <span class="Card-linkText">${__('More on Instagram')} <span class="Card-arrow"></span></span>
          </a>
        </div>
      </article>
    `
