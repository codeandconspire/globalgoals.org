...
module.exports = function single (state, emit, doc, opts = {}) {
  ...
  if (!doc.tags.includes('organisation') && !state.goals.error) {
    for (const tag of taggedGoals) {
      ...
      if (!goal) {
        emit('goals:fetch', { number: tag, critical: false })
      }
      goals.push(goal || tag)
    }
  }
                ...
                <div class="Single-block Single-block--inset">
                  ...
                  <div class="Text">
                    <small class="Text-muted">
                      ...
                      ${goals.map(item => {
                        return item.id ? html`<span>${__('Goal %s', item.data.number)} – ${asText(item.data.title)}</span>` : __('LOADING_TEXT_SHORT')
                ...
                <div class="Single-block">
                  ...
                  <div class="Single-line">
                    ${shareable({
                      large: true,
                      email: true,
                      href: `mailto:?subject=${asText(doc.data.title).trim()}&body=${asText(doc.data.introduction).trim()} ${url.resolve(ROOT, href(state, doc)).replace(/\/$/, '')}`
                    })}
                  </div>
                </div>
