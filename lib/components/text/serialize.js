    ...
    case Elements.embed: {
      ...
      switch (provider && provider.toLowerCase()) {
        ...
        case 'vimeo': return html`
          <div class="${className('Text Text-embed', {[`Text-embed--${provider.toLowerCase()}`]: provider})}">
            ${raw(element.oembed.html.replace(/<\\\//g, '</'))}
          </div>
        `
        default: return html`
          <div class="${className('Text Text-embed', {[`Text-embed--${provider && provider.toLowerCase()}`]: provider})}">
