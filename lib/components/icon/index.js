...
const label = (num, componentName) => createLabel(num, componentName);
const glyph = (num, componentName) => html`
  <div class="Icon-glyph ${ componentName }-iconGlyph">${ ICONS[num - 1] }</div>
`;

const icon = ({ goal = 1, cover = false, componentName = 'Icon' } = {}) => {
  return html`
    <div class="Icon Icon--${ goal } ${ cover ? 'Icon--cover' : '' }">
      ${ label(goal, componentName) }
      ${ glyph(goal, componentName)}
    </div>
  `;
};
...
function createLabel(number, componentName = 'Icon') {
  ...
  return html`
    <svg class="Icon-label ${ componentName }-iconLabel" viewBox="0 0 200 ${ viewBoxHeight }">
      <g style="transform: scale(0.94)">
        <text class="Icon-number ${ componentName }-iconNumber" font-size="59.4" x="${ digitPos }" y="24.2" fill="currentColor" text-anchor="middle" alignment-baseline="middle">${ number }</text>
