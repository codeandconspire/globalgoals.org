...
const AR_NUMBERS = '۰۱۲۳۴۵۶۷۸۹'
const AR_LANGCODE = 'ar-eg'
...
function loading (number, customClass) {
  ...
  return html`
    <div class="Icon Icon--${number}">
      ...
      <svg role="presentational" aria-hidden="true" class="${className('Icon-label', {[`${customClass}-iconLabel`]: customClass})}" viewBox="0 0 200 ${height}" style="-ms-flex: 1 1 ${height}px;" preserveAspectRatio="xMidYMin meet">
...
function draw (number, lines, lang, customClass) {
  ...
  const isArabic = lang === AR_LANGCODE
  ...
  return html`
    <svg role="presentational" aria-hidden="true" class="${className('Icon-label', {[`${customClass}-iconLabel`]: customClass, 'u-rtl': isArabic})}" height="${height * 0.92}" viewBox="0 0 200 ${height}" style="-ms-flex: 1 1 ${height}px;" preserveAspectRatio="xMidYMin meet">
      <g transform="scale(0.94)">
        <text class="${className('Icon-number', {[`${customClass}-iconNumber`]: customClass})}" font-size="59.4" fill="currentColor" text-anchor="middle" alignment-baseline="hanging">
