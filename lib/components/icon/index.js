...
const label = (goal, text = [], componentName) => {
  return html`
    <div class="Icon Icon--${goal}">
      ${draw(goal, text, componentName)}
    </div>
  `
}
...
const icon = ({ goal = 1, text = [], componentName = 'Icon' } = {}) => {
  return html`
    <div class="Icon Icon--${goal}">
      ${draw(goal, text, componentName)}
      <div class="Icon-glyph ${componentName}-iconGlyph">${ICONS[goal - 1]}</div>
...
exports.parseIconText = function parseIconText (doc) {
  if (!doc || !doc.data.icon_text) {
    return null
  }

  let result = []

  doc.data.icon_text.forEach(({ text }) => {
...
function draw (number, lines, componentName = 'Icon') {
