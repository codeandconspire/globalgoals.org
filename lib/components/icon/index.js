...
function glyph (goal, parent) {
  return html`
    <div class="Icon Icon--${goal}">
      <div class="Icon-glyph ${parent ? `${parent}-iconGlyph` : ''}">${ICONS[goal - 1]()}</div>
    </div>
  `
}
...
function icon (doc, parent) {
  return html`
    <div class="Icon Icon--${doc.data.number}">
      ${draw(doc.data.number, parseText(doc), doc.lang, parent)}
      <div class="Icon-glyph ${parent ? `${parent}-iconGlyph` : ''}">${ICONS[doc.data.number - 1]()}</div>
    </div>
  `
}
...
function draw (number, lines, lang, parent) {
  ...
  return html`
    <svg role="presentational" aria-hidden="true" class="${className('Icon-label', {[`${parent}-iconLabel`]: parent})}" height="${height * 0.92}" viewBox="0 0 200 ${height}" style="-ms-flex: 1 1 ${height}px;" preserveAspectRatio="xMidYMin meet">
