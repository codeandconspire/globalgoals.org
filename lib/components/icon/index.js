...
function draw (number, lines, lang, parent) {
  ...
  const viewBoxHeight = 48 + ((lines.length - 1) * 24)
  const longLine = lines.find(function (line) {
    return line.length > ((LANG_MULTIPLIERS[lang] || 1) * 14)
  })
  const longerLine = lines.find(function (line) {
    return line.length > ((LANG_MULTIPLIERS[lang] || 1) * 17)
  })
  ...
  return html`
    <svg role="presentational" aria-hidden="true" class="${className('Icon-label', {[`${parent}-iconLabel`]: parent})}" viewBox="0 0 200 ${viewBoxHeight}">
      <g transform="scale(0.94)">
        ...
        <text class="${className('Icon-text', {[`${parent}-iconText`]: parent})}" font-size="${(LANG_MULTIPLIERS[lang] || 1) * 24}" y="-7" fill="currentColor" text-anchor="start" alignment-baseline="hanging">
          ${lines.map(line => {
...
function lineStyles (line, lang) {
  let longLine = line.length > ((LANG_MULTIPLIERS[lang] || 1) * 14)
  let longerLine = line.length > ((LANG_MULTIPLIERS[lang] || 1) * 19)
