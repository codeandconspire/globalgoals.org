...
module.exports = class Slices extends Component {
  ...
  asRow (slices) {
    ...
    return html`
      <div class="Grid">
        ${rows.reduce((grid, row, index) => grid.concat(row.reverse().map((slice, place) => {
    ...
    switch (slice.slice_type) {
      ...
      case 'embed': return html`
        <div class="Text Text--full">
          ${asElement(slice.primary.content, href, serialize)}
        </div>
      `

      case 'video_grid': return html`
        <div class="Grid">
...
function page (doc, props) {
  ...
  if (doc.data.redirect.link_type === 'Document') {
    ...
    linkText = __('Go to website')
  } else if (doc.data.redirect.link_type === 'Media') {
    file = doc.data.redirect.url
    linkText = __('Download')
  } else {
    url = href(doc)
  }
...
function defaults (props) {
  ...
  if (props.body && props.body.length) {
