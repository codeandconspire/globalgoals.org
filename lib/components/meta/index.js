...
function update (state) {
  Object.keys(TAGS).forEach(key => {
    ...
    if (tag) {
      tag.setAttribute('content', TAGS[key](state))
    }
  })
}
...
function theme (state) {
  return state.params.goal ? color(state.params.goal) : '#ffffff'
}
...
function getDocument (state) {
  switch (state.routeName) {
    case 'home': return state.pages.items.find(item => {
      return item.type === 'home_page'
    })
    case 'news':
    case 'activities': return state.pages.items.find(item => {
    ...
    case 'activity': return state.activities.items.find(item => {
      return item.uid === state.params.activity
    })
