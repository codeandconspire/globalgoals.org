...
module.exports = component({
  ...
  render(tweets, emit, { user, hashtag }) {
    ...
    return html`
      <article class="Card Card--fill Card--noFillLink">
        <div class="Card-content u-bgTwitter">
          <div class="Twitter">
          <svg class="Twitter-icon" viewBox="0 0 36 30">
                ...
                ${ tweets ? tweets.map(tweet => {
                  ...
                  return html`
                    <section class="Twitter-tweet">
                      <a class="Twitter-user" href="https://twitter.com/${ user }">@${ user }</a>
                      <a class="Twitter-link" href="https://twitter.com/${ user }/status/${ tweet.id_str }"><time class="Twitter-date" datetime="${ JSON.stringify(date) }">${ time(+date) }</time></a>
          ...
          <a class="Card-link" href="${ link }" target="_blank" rel="noopener noreferrer">
            <span class="Card-linkText">${ __('See all Tweets') } <div class="Card-arrow"></div></span>
          </a>
        </div>
      </article>
    `;
...
function text(tweet) {
  ...
  let text = [];
  let char = 0;
  for (const part of parts.filter(part => part.indices[1] <= 140)) {
    let href, link;
    if (part.screen_name) {
