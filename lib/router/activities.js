...
router.get('/activities', async ctx => {
  ...
  await Promise.all([
    ...
    ctx.prismic.query(
      ...
      ctx.state.activities.total = response.total_results_size
      ctx.state.activities.items = response.results
    }),
    // Page header
    ctx.prismic.getByUID('landing_page', 'activities').then(doc => {
      ctx.state.pages.items.push(doc)
    })
  ])
})

router.get('/activities/:activity', async ctx => {
  ...
  const doc = await ctx.prismic.query(
    ...
    activities.items.push(response.results[0])
    activities.total = response.total_results_size
