...
router.get(GOAL_URL, match('goal'), async ctx => {
    ...
    await ctx.prismic.query([
      ...
      if (response.results_size < (GRID_SIZE - grid.length)) {
        return ctx.prismic.query([
          ...
          ctx.state.activities.tags.push(tag)
        })
      }
    })
  }
...
router.get(`${GOAL_URL}/organisations`, match('goal_organisations'), async ctx => {
...
router.get(`${GOAL_URL}/media`, match('goal_media'))
router.get(`${GOAL_URL}/tips-tricks`, match('goal_tips'))
router.get(`${GOAL_URL}/newsletter`, match('goal_newsletter'))

/**
 * All goals have a sub-route for each sharable media item
 */

router.get(`${GOAL_URL}/media/:media`, match('goal_media'), async ctx => {
  ctx.assert(ctx.state.lang === 'en', 404)
