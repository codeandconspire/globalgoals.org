...
router.get('initiative', '/initiatives/:initiative', async (ctx, next) => {
  ...
  await ctx.prismic.query(
    // Prismic.Predicates.any('my.goal.number', goalTags(ctx.state.initiatives))
    // TODO: Use Predicats when prismic accepts PR
    `[:d = any(my.goal.number, [${ goalTags(initiatives.items).join(',') }])]`
  ).then(body => {
...
/**
 * Get a list of all goals that are tagged in fiven list of documents
 *
