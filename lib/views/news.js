...
function news(state, emit) {
  const { articles: { isLoading, items, page, pageSize }} = state;
  ...
  if (!isLoading && items.length < (page * pageSize)) {
    emit('articles:fetch');
  }
  ...
  return html`
    <main class="View-main View-animationFriendly">
      <section class="View-section">
        ${ (doc) && 'data' in doc ? intro({ title: asText(doc.data.title), body: asElement(doc.data.intro, doc => href(state, doc)) }) : intro() }

        ${ items.length >= pageSize ? html`
          <div class="Grid">
