...
function news (state, emit) {
  ...
  const ids = articles.pages.reduce((all, page) => all.concat(page), [])
  ...
  const deduct = articles.page === max ? 0 : (ids.length - 2) % 3
        ...
        ${!articles.isLoading && hasAllPages ? html`
          <div class="Grid">
            ...
            ${ids.slice(2, ids.length - deduct).map(byId).map((doc, index) => {
              ...
              let position = index + 2 - ((page - 1) * articles.pageSize)
            ...
            ${articles.total && (page * articles.pageSize) < articles.total ? html`
              <div class="Grid-cell Grid-cell--lg1of3 Grid-cell--md1of2 Grid-cell--center">
                <a class="Button Button--fill Button--secondary" role="button" onclick=${paginate(page + 1)} href="${routes.news}?page=${articles.page + 1}">
