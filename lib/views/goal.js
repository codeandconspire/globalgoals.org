...
const { isBrowser } = require('../components/base/utils');
...
function goal(state, emit) {
    ...
    if (length) {
      ...
      if (grid.length < GRID_SIZE) {
        ...
        if (grid.length < GRID_SIZE && !state.activities.fetched.includes(tag)) {
          emit('activities:fetch', { tags: [ tag ]});
        }
      }
  ...
  return html`
    <main class="View-main" id="view-main">
      ...
      ${ doc ? slices(state, emit, body.map(slice => {
        switch (slice.slice_type) {
          case 'call_to_action': return callToAction(state, emit, { doc, heading: {
