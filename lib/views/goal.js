...
function goal (state, emit, render) {
  ...
  if (body) {
    ...
    const grid = body.filter(slice => GRID_SLICE.test(slice.slice_type))
    ...
    if (length) {
      ...
      /**
       * Fill up grid with articles tagged with goal
       */
  ...
  return html`
    <main class="View-main" id="view-main">
      ...
      ${doc && doc.data.manifest.length ? html`
        ...
        <section class="View-section u-transformTarget" id="targets">
          <div class="Space Space--textBlock Space--textBlockFirst">
            <div class="Text Text--growing">
              ${doc.data.targets_title ? html`<h2 class="Text-h2">${asText(doc.data.targets_title)}</h2>` : null}
              ${doc.data.targets_introduction ? html`<p>${asText(doc.data.targets_introduction)}</p>` : null}
            </div>
          </div>
