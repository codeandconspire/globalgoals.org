...
function resources (state, emit, render) {
  ...
  return html`
    <main class="View-main u-transformTarget" id="view-main">
      ...
      ${doc ? render(Slices, doc.id, doc.data.body.map((slice, index) => {
...
function resource (item) {
  return {
    ...
    file: item.file.url ? item.file.url : false,
    href: item.document.uid ? '/' + item.document.uid : false,
    sizes: ['small', 'medium'],
    link: linkText(item)
  }
}
...
function linkText (item) {
  if (!item.file.name && item.document.uid) {
    return __('Go to section')
  }
