...
const engager = createEngager('home')

module.exports = view('home', home)
...
function home (state, emit) {
  ...
  if (!goals.isLoading && !goals.error && goals.items.length !== goals.total) {
    const missing = []
  ...
  function getIntro () {
    if (doc && 'data' in doc) {
      return intro({
        ...
        body: asElement(doc.data.introduction, href),
        pageIntro: true
      })
    } else {
        ...
        switch (slice.slice_type) {
          case 'call_to_action': return engager(state, emit, { doc,
            heading: {
              ...
              introduction: asElement(slice.primary.introduction, href)
            }})
