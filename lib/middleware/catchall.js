...
module.exports = async (ctx, next) => {
  try {
    ...
    if (!ctx.body) {
      ...
      const doc = await ctx.prismic.getByUID('page', ctx.path.substr(1));
      ...
      if (!ctx.body) {
        const err = new Error(__('The link you visited did not match with anything on this website. Maybe we messed up, or there is a typo in the link.'));
        err.status = 404;
        err.expose = true;
        throw err;
      }
