...
function clear () {
  return Promise.all([
    idbKeyval.keys().then(keys => {
      return Promise.all(keys.filter(key => DB_PATTERN.test(key)).map(key => {
        if (key !== BUILD_VERSION) return idbKeyval.delete(key)
      }))
    }),
...
function render (href) {
  return state.get().then(function (initialState) {
