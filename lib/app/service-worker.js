...
const PRISMIC_REGEX = /cdn\.prismic\.io(?!.*\.\w+$).*$/
  ...
  event.respondWith(
    caches.open(BUILD_VERSION).then(cache => {
      return cache.match(event.request).then(cached => {
        ...
        const isPrismic = PRISMIC_REGEX.test(url)

        // Always bypass cache for requests to application api
        if (isApi || isPrismic || isHTML || IS_DEVELOPMENT) {
        ...
        if (mode === 'navigate' || (method === 'GET' && isHTML && initialState)) {
          ...
          return new self.Response(document(initialState, body), { headers: {
