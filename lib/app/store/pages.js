...
const { fromError, statusError } = require('../../components/base/utils')
  ...
  return function (state, emitter) {
    ...
    emitter.on('pages:fetch', batch((data, query) => {
      ...
      query(predicates, opts).then(response => {
        if (data && (data.id || data.uid || data.single) && !response.results_size) {
          throw statusError(404, __('Page not found'))
        }
        state.pages.error = null
        state.pages.isLoading = false
        state.pages.items = unique(state.pages.items.concat(sanitize(response.results)))
      }).catch(err => {
        state.pages.isLoading = false
