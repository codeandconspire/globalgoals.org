        ...
        navigator.serviceWorker.register('/service-worker.js').then(registration => {
          registration.onupdatefound = function () {
            ...
            installingWorker.onstatechange = function () {
              if (installingWorker.state !== 'redundant') {
                registration.active.postMessage({type: 'update', state: state})
              }
            }
          }
          if (registration.active) {
            registration.active.postMessage({state: state, type: 'update'})
          }
          ...
          emitter.on('*', event => {
            ...
            if (event === RENDER || event === DOMTITLECHANGE) {
              if (registration.active && !state.error) {
                if (cancel) cancel()
                cancel = onidle(function () {
                  registration.active.postMessage({state: state, type: 'update'})
                }, BATCH_PROCESS_TIMEOUT)
              }
            }
          ...
          window.fetch('/api', {
            ...
            if (next.version > state.version) {
              ...
              registration.active.postMessage({type: 'reset', state: next})
